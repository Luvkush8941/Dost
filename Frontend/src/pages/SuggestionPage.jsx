...
const SuggestionPage = () => {
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      try {
        setLoading(true);
        const res = await axios.get("http://localhost:3000/api/v1/users/suggest", {headers: {
  ...
  const handleRequestFriendship = async (userId) => {
    try {
      const res = await axios.post(`${baseUrl}/api/v1/users/friend-request`, { userId }, {headers: {
        'Access-Control-Allow-Origin': '*', 
        'Content-Type': 'application/json'
    }, withCredentials: true });
  ...
  const handleRemoveFromSuggestions = async (userId) => {
    try {
      const res = await axios.delete(`${baseUrl}/api/v1/users/suggest/${userId}`, {headers: {
        'Access-Control-Allow-Origin': '*', 
        'Content-Type': 'application/json'
    }, withCredentials: true });
